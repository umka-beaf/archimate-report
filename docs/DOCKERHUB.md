...
| `LOG_LEVEL` | no | `DEBUG` / `INFO` (default) / `WARNING` / `ERROR` — verbosity of the service's own log lines |
...
| `LOG_LEVEL` | нет | `DEBUG` / `INFO` (по умолчанию) / `WARNING` / `ERROR` — детализация собственных логов сервиса |
