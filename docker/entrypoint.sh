...
# See docs/LOGGING.md — LOG_LEVEL-aware log_debug/log_info/log_warn/log_error,
# shared with generate.sh/model-config.sh (and mirrored, since it can't
# source a bash file, in archi-webhook's logf()) so `docker logs` output from
# all processes sorts/greps consistently under one shared format.
...
log_info "starting caddy on :${PORT:-3000}"
