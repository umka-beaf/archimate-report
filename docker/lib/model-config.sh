...
# generate.sh (see CLAUDE.md §32 for the design this implements). Not meant to
...
# numbering (MODEL_1_*, MODEL_7_*, ... is fine), see §32.1.
#
# Uses its own mc_log()/mc_die() (same UTC-timestamped format as
# entrypoint.sh's log()/generate.sh's log()/die(), see §21.3) under a
# distinct "[model-config]" tag rather than overwriting the caller's own
# log()/die() — this file is sourced into two different processes that each
# already define those names for their own component tag.

mc_log() { printf '%s [model-config] %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$*" >&2; }
mc_die() {
    mc_log "ERROR: $*"
    exit 1
}
