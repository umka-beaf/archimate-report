/* ===========================================================================
   docker/report-theme: vendored Markdown renderer for element/view/model
   Documentation & Purpose text. Archi's HTML Report plugin dumps that text
   raw (white-space:pre-wrap), so authors writing Markdown saw literal
   '##'/'---'/'| a | b |' syntax on the page instead of rendered HTML.
   Vendored (not a <script src>) to keep the customization surface limited to
   the same 4 static files every generated page already references.
...
/* docker/report-theme: apply the stored/detected language + theme to this
   frame's own document. Duplicated from js/model.js - every generated page
   (index.html, elements/*.html, views/*.html) is its own <html> document, so
   there's no single shared entry point without patching Archi's page
   templates; each static JS file that's already referenced by every page
   applies the preference to whichever document it happens to run in. */
...
$(document).ready(function() {
	// docker/report-theme: apply stored/detected language + theme before
	// anything else paints in this frame
	arcApplyPreferences();
	...
	// docker/report-theme: render Documentation/Purpose text as Markdown (GFM
	// tables, headers, lists, links) instead of dumping the raw source with
	// white-space:pre-wrap. marked's own GFM autolinking covers bare URLs, so
	// createLinks() is skipped here and kept only for any other .convertlinks
	// element that isn't .documentation (none observed in practice, but the
	// plugin doesn't guarantee that will stay true).
	$('.documentation.convertlinks').each(function () {
		var raw = $(this).text();
		$(this).removeAttr('style').addClass('markdown-body').html(marked.parse(raw));
	});
	$('.convertlinks').not('.documentation').each(function() {
